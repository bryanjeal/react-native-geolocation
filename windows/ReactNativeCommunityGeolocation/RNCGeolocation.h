...
struct RNCGeolocation {
	...
	static IAsyncAction
	RequestAuthorizationAsync(ReactPromise<void> promise) noexcept {
	...
	void GetCurrentPosition(GeoOptions options,
	...
	static IAsyncAction
	GetCurrentPositionAsync(GeoOptions options,
	                        ReactPromise<JSValueObject> promise) noexcept {
		auto capturedPromise = promise;
	...
	void StartObserving(GeoOptions options) noexcept {
		...
		_positionChangedEventToken = _locator.PositionChanged(
		    {this, &RNCGeolocation::OnPositionChanged});
	}

	void OnPositionChanged(Geolocator const &,
	                       PositionChangedEventArgs const &e) noexcept {
	...
	void StartObservingStatus(GeoOptions options) noexcept {
		...
		_statusChangedEventToken =
		    _locator.StatusChanged({this, &RNCGeolocation::OnStatusChanged});
	}

	void OnStatusChanged(Geolocator const &,
	                     StatusChangedEventArgs const &e) noexcept {
	...
	static Geolocator CreateGeolocator(GeoOptions options) {
		...
		uint32_t maxAge         = std::abs(std::round(options.MaximumAge));
		uint32_t reportInterval = std::max(maxAge, (uint32_t)1000);
	...
	static JSValueObject
	ToJSValueObject(winrt::Windows::Devices::Geolocation::Geocoordinate coord) {

		auto unixtime = winrt::clock::to_time_t(coord.Timestamp());

		auto result = JSValueObject{
		    {"coords",
		     JSValueObject{{"latitude", coord.Latitude()},
	...
	winrt::AsyncActionCompletedHandler MakeAsyncActionCompletedHandler(
	    winrt::Microsoft::ReactNative::ReactPromise<TPromiseResult> const
	        &promise) {
		return
		    [promise](winrt::IAsyncAction action, winrt::AsyncStatus status) {
			    if (status == winrt::AsyncStatus::Error) {
				    ...
				    error.Message =
				        "HRESULT " + errorCode.str() + ": " +
				        std::system_category().message(action.ErrorCode());
